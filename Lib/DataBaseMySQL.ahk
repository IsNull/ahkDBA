	...
	*/
	...
	InsertMany(records, tableName){
		...
		for each, record in records
		{
			...
			for column, value in record
			{
				...
				valString .= ", " this.GetQueryLiteral(value)
			}
	...
	Update(fields, constraints, tableName, safe = True) {
		if (safe) ;limitation: information_schema doesn't work with temp tables
			for k, row in this.Query("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_KEY = 'PRI' AND TABLE_NAME = " this.GetQueryLiteral(tableName)).Rows
				if (!constraints.HasKey(row[1]))
					return -1 ; error handling....
		
		WHERE := ""
		for col, val in constraints
			WHERE .= ", " this.QuoteIdentifier(col) " = " this.GetQueryLiteral(val)
		WHERE := SubStr(WHERE, 3)
		
		SET := ""
		for col, val in fields
			SET .= ", " this.QuoteIdentifier(col) " = " this.GetQueryLiteral(val)
		SET := SubStr(SET, 3)
